...
export function EditInvoiceDialog({ invoice, invoiceItems, open, onOpenChange }: EditInvoiceDialogProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      customer_rnc: invoice.customer_rnc || "",
      payment_method: invoice.payment_method,
      payment_status: invoice.payment_status,
      delivery_date: invoice.delivery_date ? new Date(invoice.delivery_date) : undefined,
      notes: invoice.notes || "",
      apply_tax: invoice.apply_tax !== undefined ? invoice.apply_tax : true,
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Ocurrió un error al actualizar la factura",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl">Editar Factura #{invoice.invoice_number}</DialogTitle>
        </DialogHeader>
